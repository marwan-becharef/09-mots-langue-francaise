"""
Module pour la manipulation et l'analyse des mots de la langue française
"""
import random
...
def cherche1(mots, start, stop, n):
    ...
    """
    return {m for m in mots if len(m) == n and m.startswith(start) and m.endswith(stop)}
...
def cherche2(mots, lstart, lmid, lstop, nmin, nmax):
    ...
    for m in mots:
        if nmin <= len(m) <= nmax:
            # Vérifie si le début est dans lstart et la fin dans lstop
            if any(m.startswith(s) for s in lstart) and any(m.endswith(s) for s in lstop):
                if any(sub in m[1:-1] for sub in lmid):
                    res.add(m)
    return res


def main():
    """Fonction principale pour vérifier le bon fonctionnement des fonctions secondaires"""
    mots = read_data(FILENAME)
    ens = ensemble_mots(FILENAME)
    print( [
    mot for mot in ["chronophage", "procrastinateur", "dangerosité", "gratifiant"] if mot in ens ] )
